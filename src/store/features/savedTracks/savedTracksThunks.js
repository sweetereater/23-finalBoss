...
export const getSavedTracks = createAsyncThunk('savedTracks/getSavedTracks', async (_, { dispatch }) => {
    ...
    try {
        ...
        savedTracks = [...savedTracks, ...response.body.items.map((item) => item.track)]

        if (response.body.items.length < amountOfTracks) {
            let requests = [];
            while (offset + limit < amountOfTracks) {
                requests.push(spotifyApi.getMySavedTracks({ offset: offset += limit, limit }))
            }
            const responses = await Promise.all(requests);
            console.log('Responses: ', responses)
            responses.forEach(response => savedTracks = [...savedTracks, ...response.body.items.map((item) => item.track)])
        }
